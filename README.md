...
deploy new binaries **without touching your custom `.conf` files**.
...
- **Console** — send worldserver commands (`.server shutdown`, `.account create`, …) over stdin, with live output.
...
- **Self-update** — the **Check for app update** button reads this repo's GitHub Releases and prompts when a
